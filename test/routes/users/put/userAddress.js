...
describe('PUT /users/:user_id/addresses/:address_id', function () {
    before(function (done) {
        getLoggedRequest(function (err, resp) {
            ...
            makeARequest = function (userID, addressID, data, 
    ...
    it('displays errors when empty form data', function (done) {
        createAddress(csrfToken, user.id,
                      request, function (err, address) {
            
            if (err) {
                return done(err);
            }

            makeARequest(user.id, address.id, {
                _csrf: csrfToken
            }, 302, request, function (err, resp) {
                if (err) {
                    return done(err);
    ...
    it('displays errors when invalid form data', function (done) {
        createAddress(csrfToken, user.id, 
            ...
            makeARequest(user.id, address.id, address,
                         302, request, function (err, resp) {
    ...
    it('redirects to user\'s address '
